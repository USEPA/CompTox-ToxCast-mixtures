...
p1_og <- ggplot_build(plotdata.func(index=ex_index, tc_lib=TRUE) +
                        guides(shape=guide_legend(override.aes=list(size=1)),
                               color=guide_legend(override.aes=list(lwd=0.5))) + 
                        ...
                        theme(legend.title=element_blank(),
                              ...
                              plot.margin = margin(t=0.5, r=0.2,b=0,l=1.2, unit = "cm"),
                              plot.title = element_blank()))
p1_og$data[[1]]$size <- 1
p1_og$data[[2]]$size <- 1
p1_og$data[[3]]$size <- 1
p1_og$data[[4]]$linewidth <- 0.5
p1_og$data[[5]]$linewidth <- 0.5
p1_og$data[[6]]$linewidth <- 0.5
p1_og$data[[7]]$linewidth <- 0.5
...
p2_og <- ggplot_build(plotlines.func(index=ex_index,mix_input=mix.info.tc,tc=TRUE) +
                      ...
                      theme(legend.title=element_blank(),
                            ...
                            plot.title = element_blank()))
p2_og$data[[1]]$linewidth <- 0.5
p2_og$data[[2]]$linewidth <- 0.5
p2_og$data[[3]]$linewidth <- 0.5
p2_og$data[[4]]$linewidth <- 0.5
p2_og$data[[5]]$linewidth <- 0.5
p2_og$data[[6]]$size <- 1
...
p3_og$data[[5]]$linewidth <- 0.5
p3_og$data[[6]]$linewidth <- 0.5
p3_og$data[[7]]$size <- 1
...
p4_og$data[[3]]$linewidth <- 0.5
p4_og$data[[4]]$size <- 1
p4_og$data[[5]]$linewidth <- 0.5
...
p1 <- ggplot(acc_plot_info, aes(x=acc, y=chnm, color=variable)) +
      ...
      geom_errorbarh(aes(xmax=acc_plot_info$boot_hi, xmin=acc_plot_info$boot_lo, height=1.1), 
                     linewidth=0.5,
...
p2 <- ggplot(acc_plot_info, aes(x=acc, y=chnm, color=variable)) +
      ...
      geom_errorbarh(aes(xmax=acc_plot_info$bayes_hi, xmin=acc_plot_info$bayes_lo, height=1.1),
...
tiff("./figures/Figure_4.tiff", units="mm", width=190, height=110, res=600)
plot_grid(
  plot_grid(p1,p2,nrow=1,ncol=2,rel_widths=c(2.6,1)),
...
p1_og$data[[1]]$linewidth <- 0.5
p1_og$data[[2]]$linewidth <- 0.5
p1_og$data[[3]]$linewidth <- 0.5
p1_og$data[[4]]$linewidth <- 0.5
p1_og$data[[5]]$linewidth <- 0.5
p1_og$data[[6]]$size <- 1
p1 <- ggplot_gtable(p1_og)
...
tiff("./figures/Figure_6.tiff", units="mm", width=90, height=120, res=600)
...
p4_og <- ggplot_build(plotlines.func(index=ex_index_3,mix_input=mix.info.tc,tc=TRUE) +
                        ...
                        theme(legend.title=element_blank(),
                              ...
                              plot.margin=margin(t=4, r=5,b=0,l=5, unit = "mm"),
                              plot.title=element_text(hjust=0.5,face="bold")))
p4_og$data[[1]]$linewidth <- 0.5
p4_og$data[[2]]$linewidth <- 0.5
p4_og$data[[3]]$linewidth <- 0.5
p4_og$data[[4]]$linewidth <- 0.5
p4_og$data[[5]]$linewidth <- 0.5
p4_og$data[[6]]$size <- 1
p4 <- ggplot_gtable(p4_og)
...
p5_og$data[[3]]$linewidth <- 0.5
p5_og$data[[4]]$size <- 1
p5_og$data[[5]]$linewidth <- 0.5
...
p6_og$data[[3]]$linewidth <- 0.5
p6_og$data[[4]]$size <- 1
p6_og$data[[5]]$linewidth <- 0.5
p6 <- ggplot_gtable(p6_og)

tiff("./figures/Figure_7.tiff", units="mm", width=190, height=150, res=600)
...
```{r}
ex_m4id <- 12635939
m4id_1 <- subset(mix.info, m4id_mix==ex_m4id)$m4id_1
m4id_2 <- subset(mix.info, m4id_mix==ex_m4id)$m4id_2
ex_index <- which(mix.results.all$m4id_mix==ex_m4id)

mix_data <- mc3 %>% filter(m3id %in% mc4_agg[m4id==ex_m4id,]$m3id)
comp1_data<- mc3 %>% filter(m3id %in% mc4_agg[m4id==m4id_1,]$m3id)
comp2_data <- mc3 %>% filter(m3id %in% mc4_agg[m4id==m4id_2,]$m3id)
comp1_plot <- data.frame(x=seq(min(comp1_data$conc),max(comp1_data$conc),length.out=300),
                     resp=toxcast_model(dat=mc5[m4id==m4id_1,],
                                        params=mc5[m4id==m4id_1,],
                                        XX=seq(min(comp1_data$conc),
                                               max(comp1_data$conc),
                                               length.out=300)))
comp2_plot <- data.frame(x=seq(min(comp2_data$conc),max(comp2_data$conc),length.out=300),
                     resp=toxcast_model(dat=mc5[m4id==m4id_2,],
                                        params=mc5[m4id==m4id_2,],
                                        XX=seq(min(comp2_data$conc),
                                               max(comp2_data$conc),
                                               length.out=300)))
...
cutoff <- mix.info$coff[ex_index]
norm_factor <- mix.info$edpt_top[ex_index]
...
p_outline <- ggplot() +
  ...
  theme(axis.text = element_text(color="black"),
        plot.margin=margin(t=0.6, r=0.5,b=0.6,l=0.1, unit = "cm"),
        plot.subtitle=element_text(hjust=0.5, size=11)) 

# tcplfit2 best fits
p_comp1 <- p_outline +
    geom_point(data=comp1_data, aes(x = conc, y = resp/norm_factor),color="black",shape=15, size=1) +
    geom_line(data = comp1_plot, aes(x=x,y=resp/norm_factor),color="#D55E00", lwd=0.5) +
    labs(title="Component 1") +
    geom_hline(yintercept=cutoff/norm_factor, linetype="dashed", color="gray34",linewidth=0.5) +
    theme(plot.title=element_text(size=12,hjust=0.5, face="bold", color="#D55E00")) 

p_comp2 <- p_outline +
    geom_point(data=comp2_data, aes(x = conc, y = resp/norm_factor),color="black",shape=17, size=1) +
    geom_line(data = comp2_plot, aes(x=x,y=resp/norm_factor),color="#009E73", lwd=0.5) +
    geom_hline(yintercept=cutoff/norm_factor, linetype="dashed", color="gray34",linewidth=0.5) +
    labs(title="Component 2")+
    theme(plot.title=element_text(size=12,hjust=0.5, face="bold", color="#009E73")) 

p_mix <- ggplot_build(plotlines.func(index=ex_index,mix_input=mix.info,tc=FALSE) +
                      guides(color=guide_legend(override.aes=list(lwd=0.75,size=1.25))) +
                      theme_bw(base_size=9) +
                      labs(title="Mathematical Models for", subtitle="Simulating Mixtures")+
                      scale_color_manual(values=c(CA="#ffb000",IA="#006299",
                                                  Single="#be548f",Mixture="black"),
                         breaks=c("Mixture","CA","IA","Single"),
                         labels=c(CA="Concentration Addition",IA="Independent Action",Single="Most Potent Single Component",Mixture="Observed Mixture"))+
                      theme(legend.title=element_blank(),
                            legend.text = element_text(size=8),
                            legend.background = element_rect(fill=NA),
                            legend.margin = margin(0.1,0.1,0.1,0.1, unit='mm'),
                            ...
                            axis.text = element_text(color="black"),
                            plot.margin = margin(t=0.3, r=0.3,b=0.3,l=0.6, unit = "cm"),
                            plot.title = element_text(size=14,hjust=0.5, face="bold"),
                            plot.subtitle = element_text(size=14,hjust=0.5, face="bold")))
p_mix$data[[1]]$linewidth <- 0.75
p_mix$data[[2]]$linewidth <- 0.75
p_mix$data[[3]]$linewidth <- 0.75
p_mix$data[[4]]$linewidth <- 0.75
p_mix$data[[5]]$linewidth <- 0.75
p_mix$data[[6]]$size <- 1.25
p_mix <- ggplot_gtable(p_mix)
  

p_boot <- ggplot_build(plotint.func(index=ex_index,tc=FALSE) +
                      # guides(color=guide_legend(override.aes=list(lwd=0.5,size=1)),
                      #        fill=guide_legend(override.aes=list(lwd=0.5))) +
                      theme_bw(base_size=8) +
                      labs(title="Bootstrap Resampling", subtitle="Uncertainty Quantification")+
                      # scale_fill_manual(values=c(CA="#ffb000",IA="#006299",
                      #                            Single="#be548f",Mix_Int="gray40"),
                      #   breaks=c("Mix_Int","CA","IA","Single"),
                      #   labels=c(CA="Concentration Addition",IA="Independent Action",Single="Most Potent Single Component",Mix_Int="Observed Mixture"))+
                      ...
                      theme(legend.title=element_blank(),
                            ...
                            legend.spacing.y = unit(-0.1, "mm"),
                            axis.text = element_text(color="black"),
                            plot.margin = margin(t=0.2, r=0.5,b=0.2,l=0.6, unit = "cm"),
                            plot.title = element_text(size=12,hjust=0.5, face="bold"),
                            plot.subtitle = element_text(size=12,hjust=0.5, face="bold")))
p_boot$data[[1]]$linewidth <- 0.5
p_boot$data[[2]]$linewidth <- 0.5
p_boot$data[[3]]$linewidth <- 0.5
p_boot$data[[4]]$linewidth <- 0.5
p_boot$data[[5]]$linewidth <- 0.5
p_boot$data[[6]]$linewidth <- 0.5
p_boot$data[[7]]$size <- 1
p_boot <- ggplot_gtable(p_boot)

p_bayes <- ggplot_build(plotbayesintia.func(index=ex_index,tc=FALSE) +
                      # guides(color=guide_legend(override.aes=list(lwd=0.5,size=1))) +
                      guides(color = "none",fill = "none")+
                      theme_bw(base_size=8) +
                      labs(title="Bayesian Approach", subtitle="Uncertainty Quantification")+
                      theme(legend.title=element_blank(),
                            legend.text = element_text(size=7),
                            legend.background = element_rect(fill=NA),
                            legend.margin = margin(0.1,0.1,0.1,0.1, unit='mm'),
...
p_bayes$data[[3]]$linewidth <- 0.5
p_bayes$data[[4]]$size <- 1
p_bayes$data[[5]]$linewidth <- 0.5
p_bayes <- ggplot_gtable(p_bayes)


tiff("./figures/GraphicalAbstract.tiff", units="mm", width=190, height=100, res=600)
plot_grid(
  plot_grid(p_comp1,p_comp2, nrow=2)+
    annotate("text", x=0.55, y=0.5, size=16,label="+"),
  plot_grid(NULL,p_mix,NULL,nrow=3,rel_heights=c(0.15,1,0.15)),
  plot_grid(p_boot,p_bayes,nrow=2),
  ncol=3, rel_widths=c(0.6,0.9,0.8)
)+
annotate("segment",linewidth=1,x=0.21,y=0.78,xend=0.34,yend=0.73,arrow=arrow())+
annotate("segment",linewidth=1,x=0.21,y=0.17,xend=0.34,yend=0.23,arrow=arrow())
dev.off()


png("./figures/GraphicalAbstract.png", units="mm", width=170, height=100, res=600)
plot_grid(
  plot_grid(p_comp1,p_comp2, nrow=2)+
    annotate("text", x=0.55, y=0.5, size=16,label="+"),
  plot_grid(NULL,p_mix,NULL,nrow=3,rel_heights=c(0.15,1,0.15)),
  plot_grid(p_boot,p_bayes,nrow=2),
  ncol=3, rel_widths=c(0.49,0.91,0.82)
)+
annotate("segment",linewidth=1,x=0.18,y=0.78,xend=0.31,yend=0.73,arrow=arrow())+
annotate("segment",linewidth=1,x=0.18,y=0.17,xend=0.31,yend=0.23,arrow=arrow())
dev.off()
...
dataset_names <- list('mc5_mc6_mixtures' = mc5_mixture, 
                      ...
                      'methods' = methods,
                      'method_list' = method_list)
write_xlsx(dataset_names,"./figures/Supplemental_File_4_tcploutput.xlsx")
```
