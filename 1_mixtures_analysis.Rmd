...
```{r,eval=FALSE}
# select single chemicals in ToxCast not from mixture experiment
tc_mc5_notmix <- mc5_attg_endpts %>% filter(!(spid %in% spids))
tc_mc5_notmix_filter <- tcplSubsetChid(tc_mc5_notmix)


mc5.single.tc <- data.frame(matrix(NA,nrow=nrow(mc5.single), ncol=ncol(tc_mc5_notmix_filter)))
for (ii in 1:nrow(mc5.single)){
  mc5.single.tc[ii,] <- tc_mc5_notmix_filter %>% filter(dsstox_substance_id==mc5.single$dsstox_substance_id[ii] & aeid==mc5.single$aeid[ii])
}
colnames(mc5.single.tc) <- colnames(tc_mc5_notmix_filter)
setDT(mc5.single.tc)

spids_tc <- unique(mc5.single.tc$spid)
...
mix.info <- data.frame(m4id_mix=mc5.mixture.filter$m4id, modl=mc5.mixture.filter$modl, top=mc5.mixture.filter$top, aenm=mc5.mixture.filter$aenm, aeid=mc5.mixture.filter$aeid, chnm=mc5.mixture.filter$chnm, hitc=mc5.mixture.filter$hitc, coff=mc5.mixture.filter$coff, acc=mc5.mixture.filter$acc, bmad=mc5.mixture.filter$bmad, dtxsid=mc5.mixture.filter$dsstox_substance_id, spid=mc5.mixture.filter$spid, fitc=mc5.mixture.filter$fitc, m4id_1=NA, m4id_2=NA, cfrac_1= NA, cfrac_2 = NA)
...
for (qq in 1:nrow(mc5.mixture.filter)){
  ...
  mix.info$m4id_1[qq] <- mc5.single[(dsstox_substance_id==dtxsids[1] & aeid == mc5.mixture.filter$aeid[qq]),]$m4id
  mix.info$m4id_2[qq] <- mc5.single[(dsstox_substance_id==dtxsids[2] & aeid == mc5.mixture.filter$aeid[qq]),]$m4id
...
single.info.full <- data.frame(m4id=mc5.single$m4id, modl=mc5.single$modl,
                          top=mc5.single$top, hitc=mc5.single$hitc,
                          aeid=mc5.single$aeid, aenm=mc5.single$aenm,
                          chnm=mc5.single$chnm, bmad=mc5.single$bmad,
                          coff=mc5.single$coff,
                          dtxsid=mc5.single$dsstox_substance_id,
                          spid=mc5.single$spid, fitc=mc5.single$fitc,
                          acc=mc5.single$acc)

# select mixtures with at least one active component and positive tops
single_comp_active <- mc5.single[hitc >= 0.9 & top>=0]
...
single_comp_40 <- mc5.single %>% filter(fitc == 40)
...
mix.info.tc <- data.frame(m4id_mix=mc5.mixture.filter$m4id, modl=mc5.mixture.filter$modl, top=mc5.mixture.filter$top, aenm=mc5.mixture.filter$aenm, aeid=mc5.mixture.filter$aeid, chnm=mc5.mixture.filter$chnm, hitc=mc5.mixture.filter$hitc, coff=mc5.mixture.filter$coff, acc=mc5.mixture.filter$acc, bmad=mc5.mixture.filter$bmad, dtxsid=mc5.mixture.filter$dsstox_substance_id, spid=mc5.mixture.filter$spid, fitc=mc5.mixture.filter$fitc, m4id_1=NA, m4id_2=NA, cfrac_1= NA, cfrac_2 = NA)

for (qq in 1:nrow(mc5.mixture.filter)){
  # relate mixture to single components and identify by m4id
  dtxsids <- mixtures_key[mixtures_key$DTXSID.x == mc5.mixture.filter$dsstox_substance_id[qq], ]$DTXSID.y
  mix.info.tc$m4id_1[qq] <- mc5.single.tc[(dsstox_substance_id==dtxsids[1] & aeid == mc5.mixture.filter$aeid[qq]),]$m4id
  mix.info.tc$m4id_2[qq] <- mc5.single.tc[(dsstox_substance_id==dtxsids[2] & aeid == mc5.mixture.filter$aeid[qq]),]$m4id
  # save concentration fractions
  ci_frac <- mixtures_key[mixtures_key$DTXSID.x == mc5.mixture.filter$dsstox_substance_id[qq], ]$cfrac_mix
...
single.info.full.tc <- data.frame(m4id=mc5.single.tc$m4id, modl=mc5.single.tc$modl,
                          top=mc5.single.tc$top, hitc=mc5.single.tc$hitc,
                          aeid=mc5.single.tc$aeid, aenm=mc5.single.tc$aenm,
                          chnm=mc5.single.tc$chnm, bmad=mc5.single.tc$bmad,
                          coff=mc5.single.tc$coff,
                          dtxsid=mc5.single.tc$dsstox_substance_id,
                          spid=mc5.single.tc$spid, fitc=mc5.single.tc$fitc,
                          acc=mc5.single.tc$acc, bootparams=NA)

# select mixtures with at least one active component and positive tops
single_comp_active_tc <- mc5.single.tc[hitc >= 0.9 & top>=0]
mix.info.tc <- subset(mix.info.tc,((m4id_1 %in% single_comp_active_tc$m4id)|(m4id_2 %in% single_comp_active_tc$m4id)))

# remove mixtures with single components that are just at the maximum response and do not capture the slope/AC50
single_comp_40_tc <- mc5.single.tc %>% filter(fitc == 40)
...
for (ii in 1:nrow(mix.info.tc)){
  mix.info.tc$condition1[ii] <- ifelse(mc5_attg_endpts[m4id==mix.info.tc$m4id_1[ii],]$hitc>=0.9,TRUE,FALSE)
  mix.info.tc$condition2[ii] <- ifelse(mc5_attg_endpts[m4id==mix.info.tc$m4id_2[ii],]$hitc>=0.9,TRUE,FALSE)
}
```
...
for (jj in 1:nrow(single.info)){
  if (single.info$hitc[jj]>=0.9 & single.info$modl[jj]=='poly2'){
    ...
    if(mc5[m4id==single.info$m4id[jj],]$a > 0 & mc5[m4id==single.info$m4id[jj],]$b < 0){
      ...
      modl_params$bootparams <- list(curve_bootstrap(jj=1, curve.info=modl_params, boot_num=1000,type="single",tc=FALSE))
      # refit with Bayesian approach
      modl_params$bayesparams <- list(bayes_curve(ii=1,curve.info=modl_params, n_iter=10000, n_burn=2000, type="single", poly2_correct=TRUE, tc=FALSE))
...
for (jj in 1:nrow(single.info.tc)){
  if (single.info.tc$hitc[jj]>=0.9 & single.info.tc$modl[jj]=='poly2'){
    # if the poly2 curve meets the parameter conditions for concave up (a>0 and b<0), then select a different curve shape
    if(mc5_attg_endpts[m4id==single.info.tc$m4id[jj],]$a > 0 & mc5_attg_endpts[m4id==single.info.tc$m4id[jj],]$b < 0){
      aics <- dplyr::select(filter(mc4_tcsubset,m4id==single.info.tc$m4id[jj]),ends_with("aic"))
      remaining_models <- dplyr::select(aics,!c("poly2_aic","cnst_aic"))
      new_modl <- str_remove(names(which.min(remaining_models)),"_aic")
      modl_params <- dplyr::select(filter(mc4_tcsubset,m4id==single.info.tc$m4id[jj]),starts_with(new_modl))
      colnames(modl_params) <- str_remove(names(modl_params),paste0(new_modl,"_"))
      modl_params$modl <- new_modl
      ...
      # refit with bootstrap resampling
      modl_params$bootparams <- list(curve_bootstrap(jj=1, curve.info=modl_params, boot_num=1000,type="single",tc=TRUE))
      modl_params$bayesparams <- list(bayes_curve(ii=1, curve.info=modl_params, n_iter=10000, n_burn=2000, type="single", poly2_correct=TRUE, tc=TRUE))
      poly2_new_modl <- rbind(poly2_new_modl,modl_params,fill=TRUE)
    }
  }
}
...
single.info$bootparams <- lapply(c(1:nrow(single.info)), curve_bootstrap,
                                 curve.info=single.info, boot_num=1000, type="single", tc=FALSE)
single.info$bayesparams <- lapply(c(1:nrow(single.info)), bayes_curve,
                                  curve.info=single.info, n_iter=10000, n_burn=2000,
                                  type="single", poly2_correct=FALSE, tc=FALSE)

# legacy ToxCast data single curves
single.info.tc$bootparams <- lapply(c(1:nrow(single.info.tc)), curve_bootstrap,
                                 curve.info=single.info.tc, boot_num=1000, type="single", tc=TRUE)
single.info.tc$bayesparams <- lapply(c(1:nrow(single.info.tc)), bayes_curve,
                                  curve.info=single.info.tc, n_iter=10000, n_burn=2000,
                                  type="single", poly2_correct=FALSE, tc=TRUE)
# mixtures data curves
mix.info$bootparams <- lapply(c(1:nrow(mix.info)), curve_bootstrap,
                                 curve.info=mix.info, boot_num=1000, type="mix", tc=FALSE)
mix.info$bayesparams <- lapply(c(1:nrow(mix.info)), bayes_curve,
                                  curve.info=mix.info, n_iter=10000, n_burn=2000,
...
mix.results.acc$acc_mix <- mc5_attg_endpts$acc[match(mix.results.acc$m4id_mix,mc5_attg_endpts$m4id)]
...
for (ii in 1:nrow(mix.results.acc)){
  if (!(mix.results.acc$m4id_mix[ii] %in% mix.info$m4id_mix)) {
    ...
    mix.results.acc$acc_pred_ca[ii] <- NA
    mix.results.acc$acc_pred_ia[ii] <- NA
  } else {
    ...
    # bayesian prediction interval ACC CA
    acc_pred_ca <- sapply(indices,ca_point_pred,YY=comp1$coff,comp1=comp1,
                         comp2=comp2,params1_samp=bayesparams1,params2_samp=bayesparams2,
                         cfrac1=cfrac1,cfrac2=cfrac2,condition1=mix_info$condition1,
                         condition2=mix_info$condition2)
    mix.results.acc$acc_pred_ca[ii] <- list(acc_pred_ca)
    ...
    ia_resps_temp <- ia_full_resp(val=x_vals,comp1=comp1,comp2=comp2,params1=params1,params2=params2,
                             cfrac1=cfrac1,cfrac2=cfrac2,condition1=mix_info$condition1,
    ...
    if (max(ia_resps,na.rm=TRUE)<comp1$coff/mix_info$edpt_top){
      ...
      acc_ia <- approx(x=ia_resps, y=x_vals_eval, xout=comp1$coff/mix_info$edpt_top, rule=1,ties="ordered")[[2]]
    ...
    ia_boot_temp <- lapply(c(1:num_cols),ia_full_resp,val=x_vals,comp1=comp1,
                           comp2=comp2,params1=bootparams1,params2=bootparams2,
                           cfrac1=cfrac1,cfrac2=cfrac2,condition1=mix_info$condition1,
                           condition2=mix_info$condition2, norm_factor=mix_info$edpt_top,
                           sample=TRUE)
    for (ss in 1:num_cols){
      if (sum(!is.na(ia_boot_temp[[ss]]))==0){
        ...
        if (max(ia_boot,na.rm=TRUE)<comp1$coff/mix_info$edpt_top){
          ...
          acc_ia_boot_temp <- approx(x=ia_boot, y=x_vals_eval, xout=comp1$coff/mix_info$edpt_top, rule=1,ties="ordered")[[2]]
        }
      }
 ...
 # Bayesian IA
    ...
    for (ss in 1:num_sample){
      if (sum(!is.na(ia_bayes_temp[[ss]]))==0){
        ...
        if (max(ia_bayes,na.rm=TRUE)<comp1$coff/mix_info$edpt_top){
          ...
          acc_ia_bayes_temp <- approx(x=ia_bayes, y=x_vals_eval, xout=comp1$coff/mix_info$edpt_top,
    ...
    # Bayesian prediction interval IA
    ia_pred_curve <- lapply(indices, ia_full_pred, comp1=comp1,comp2=comp2,
                       cfrac1=cfrac1,cfrac2=cfrac2,params1=bayesparams1,
                       params2=bayesparams2,norm_factor=mix_info$edpt_top,
                       x_vals=x_vals,condition1=mix_info$condition1,
                       condition2=mix_info$condition2)
    acc_pred_ia <- c()
    for (zz in 1:length(indices)){
      if (sum(!is.na(ia_pred_curve[[zz]]))==0){
        acc_ia_temp <- NA
      } else{
      # ia_resps_pred_temp <- ia_pred_curve[,zz]
      # cutoff IA at top (don't include biphasic part)
      # ia_resps_pred <- ia_pred_curve[1:which.max(ia_pred_curve[,zz]),zz]
        ia_resps_pred <- ia_pred_curve[[zz]][1:which.max(ia_pred_curve[[zz]])]
        x_vals_eval <- x_vals[1:which.max(ia_pred_curve[[zz]])]
        if (max(ia_resps_pred,na.rm=TRUE)<comp1$coff/mix_info$edpt_top){
          acc_ia_temp <- 10000
        } else if (min(ia_resps_pred,na.rm=TRUE)>comp1$coff/mix_info$edpt_top){
          acc_ia_temp <- -10000
        } else {
          acc_ia_temp <- approx(x=ia_resps_pred, y=x_vals_eval, xout=comp1$coff/mix_info$edpt_top,
                           rule=1,ties="ordered")[[2]]
          # acc_ia <- approx(x=ia_resps, y=x_vals, xout=comp1$coff/mix_info$edpt_top, rule=1,ties=list("ordered",mean))[[2]]
        }
      }
      acc_pred_ia <- append(acc_pred_ia,acc_ia_temp)
    }
    mix.results.acc$acc_pred_ia[ii] <- list(acc_pred_ia)
...
for (ii in 1:nrow(mix.results.acc)){
  if (!(mix.results.acc$m4id_mix[ii] %in% mix.info.tc$m4id_mix)) {
    ...
    mix.results.acc$acc_pred_ca.tc[ii] <- NA
    mix.results.acc$acc_pred_ia.tc[ii] <- NA
  } else {
    ...
    acc_pred_ca <- sapply(indices,ca_point_pred,YY=comp1$coff,comp1=comp1,
                         ...
                         cfrac1=cfrac1,cfrac2=cfrac2,condition1=mix_info$condition1,
                         condition2=mix_info$condition2)
    mix.results.acc$acc_pred_ca.tc[ii] <- list(acc_pred_ca)
    # compute full curve IA line
    if(comp1$hitc>=0.9 & comp2$hitc<0.9){
      x_vals <- 10^seq(log10(0.01*mc5[m4id==mix.results.acc$m4id_mix[ii],]$conc_min),log10(3*comp1$conc_max/cfrac1),length.out=num_pts)
    }else if(comp2$hitc>=0.9 & comp1$hitc<0.9){
 ...
 # Bayesian IA
    ...
    mix.results.acc$acc_pred_ia.tc[ii] <- list(acc_pred_ia)
...
for(jj in 1:nrow(mix.results.all)){
  ...
  if (mix.results.all$m4id_mix[jj] %in% mix.info.tc$m4id_mix){
    ...
    if ((mc5_attg_endpts[m4id == m4id_1,]$hitc >= 0.9) & (mc5_attg_endpts[m4id == m4id_2,]$hitc < 0.9)){
      potent_sing <- mc5_attg_endpts[m4id == m4id_1,]
    } else if ((mc5_attg_endpts[m4id == m4id_1,]$hitc < 0.9) & (mc5_attg_endpts[m4id == m4id_2,]$hitc >= 0.9)){
...
for (ii in 1:nrow(mix.results.all)){
  if (!(mix.results.all$m4id_mix[ii] %in% mix.info$m4id_mix)) {
    ...
    single_dat <- mc5_attg_endpts[m4id==single_info$m4id]
    ...
    single_curve <- sapply(c(1:nrow(single_info$bootparams[[1]])), toxcast_model,
                      dat=single_dat,params=single_info$bootparams[[1]],XX=x_vals,sample=TRUE)
    ...
    single_curve_bayes <- sapply(indices, toxcast_model,
                      dat=single_dat,params=single_info$bayesparams[[1]],XX=x_vals,sample=TRUE)
    single_bayesint <- apply(single_curve_bayes, 1, quantile, prob=c(0.025,0.975), na.rm=TRUE)
...
# save(mix.results.all,file="mix_results_all.Rdata")
