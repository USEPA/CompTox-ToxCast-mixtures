...
# add flags
mc6 <- tcplPrepOtpt(tcplLoadData(lvl=6, fld='spid', val=spids, type='mc'))
setDT(mc6)
...
```{r}
# Test dataset
mc5_mix_all <- mc5 %>% filter(dsstox_substance_id %in% mixtures_key$DTXSID.x)
mc5_sing <- mc5 %>% filter(dsstox_substance_id %in% mixtures_key$DTXSID.y)
# mc5_sing_tc <- tc_mc5_notmix_filter %>% filter(dsstox_substance_id %in% mixtures_key$DTXSID.y)
...
for (qq in 1:nrow(mc5_mix_all)){
  ...
  mc5_mix_all$m4id_1[qq] <- mc5[(dsstox_substance_id==dtxsids[1] & aeid == mc5_mix_all$aeid[qq]),]$m4id
  mc5_mix_all$m4id_2[qq] <- mc5[(dsstox_substance_id==dtxsids[2] & aeid == mc5_mix_all$aeid[qq]),]$m4id
  mc5_mix_all$fitc_1[qq] <- mc5[(dsstox_substance_id==dtxsids[1] & aeid == mc5_mix_all$aeid[qq]),]$fitc
  mc5_mix_all$fitc_2[qq] <- mc5[(dsstox_substance_id==dtxsids[2] & aeid == mc5_mix_all$aeid[qq]),]$fitc
}
...
mc5_mix_all <- mc5 %>% filter(dsstox_substance_id %in% mixtures_key$DTXSID.x)
mc5_sing_tc <- tc_mc5_notmix_filter %>% filter((dsstox_substance_id %in% mixtures_key$DTXSID.y) & (aeid %in% mc5_mix_all$aeid))

# sort mixtures by active and inactive single components
for (qq in 1:nrow(mc5_mix_all)){
  # relate mixture to single components and identify by m4id
  dtxsids <- mixtures_key[mixtures_key$DTXSID.x == mc5_mix_all$dsstox_substance_id[qq], ]$DTXSID.y
  mc5_mix_all$m4id_1[qq] <- mc5_sing_tc[(dsstox_substance_id==dtxsids[1] & aeid == mc5_mix_all$aeid[qq]),]$m4id
  mc5_mix_all$m4id_2[qq] <- mc5_sing_tc[(dsstox_substance_id==dtxsids[2] & aeid == mc5_mix_all$aeid[qq]),]$m4id
  mc5_mix_all$fitc_1[qq] <- mc5_sing_tc[(dsstox_substance_id==dtxsids[1] & aeid == mc5_mix_all$aeid[qq]),]$fitc
  mc5_mix_all$fitc_2[qq] <- mc5_sing_tc[(dsstox_substance_id==dtxsids[2] & aeid == mc5_mix_all$aeid[qq]),]$fitc
}
...
```{r}
mix.results.acc$acc_single <- mc5_attg_endpts$acc[match(mix.results.all$potent_sing_id,mc5_attg_endpts$m4id)]
mix.results.acc$acc_single.tc <- mc5_attg_endpts$acc[match(mix.results.all$potent_sing_id.tc,mc5_attg_endpts$m4id)]
...
# bootstrap of ACC differences
for (ii in 1:nrow(mix.results.acc)){
  if (!(fit_quality$m4id_mix[ii] %in% mix.info$m4id_mix)){
    fit_quality$diff_acc_boot_ca[ii] <- NA
    fit_quality$diff_acc_boot_ia[ii] <- NA
    fit_quality$diff_acc_boot_single[ii] <- NA
    fit_quality$diff_acc_bayes_ca[ii] <- NA
    fit_quality$diff_acc_bayes_ia[ii] <- NA
    fit_quality$diff_acc_bayes_single[ii] <- NA
  } else {
    fit_quality$diff_acc_boot_ca[ii] <- list(mix.results.acc$acc_boot_mix[[ii]]/
                                               sample(mix.results.acc$acc_boot_ca[[ii]]))
    fit_quality$diff_acc_boot_ia[ii] <- list(mix.results.acc$acc_boot_mix[[ii]]/
                                               sample(mix.results.acc$acc_boot_ia[[ii]]))
    fit_quality$diff_acc_boot_single[ii] <- list(mix.results.acc$acc_boot_mix[[ii]]/
                                               sample(mix.results.acc$acc_boot_single[[ii]]))
    fit_quality$diff_acc_bayes_ca[ii] <- list(mix.results.acc$acc_bayes_mix[[ii]]/
                                               sample(mix.results.acc$acc_bayes_ca[[ii]]))
    fit_quality$diff_acc_bayes_ia[ii] <- list(mix.results.acc$acc_bayes_mix[[ii]]/
                                               sample(mix.results.acc$acc_bayes_ia[[ii]]))
    fit_quality$diff_acc_bayes_single[ii] <- list(mix.results.acc$acc_bayes_mix[[ii]]/
                                               sample(mix.results.acc$acc_bayes_single[[ii]]))
  }
  # repeat for TC library
  if (!(fit_quality$m4id_mix[ii] %in% mix.info.tc$m4id_mix)){
    fit_quality$diff_acc_boot_ca.tc[ii] <- NA
    fit_quality$diff_acc_boot_ia.tc[ii] <- NA
    fit_quality$diff_acc_boot_single.tc[ii] <- NA
    fit_quality$diff_acc_bayes_ca.tc[ii] <- NA
    fit_quality$diff_acc_bayes_ia.tc[ii] <- NA
    fit_quality$diff_acc_bayes_single.tc[ii] <- NA
  } else {
    fit_quality$diff_acc_boot_ca.tc[ii] <- list(mix.results.acc$acc_boot_mix[[ii]]/
                                               sample(mix.results.acc$acc_boot_ca.tc[[ii]]))
    fit_quality$diff_acc_boot_ia.tc[ii] <- list(mix.results.acc$acc_boot_mix[[ii]]/
                                               sample(mix.results.acc$acc_boot_ia.tc[[ii]]))
    fit_quality$diff_acc_boot_single.tc[ii] <- list(mix.results.acc$acc_boot_mix[[ii]]/
                                               sample(mix.results.acc$acc_boot_single.tc[[ii]]))
    fit_quality$diff_acc_bayes_ca.tc[ii] <- list(mix.results.acc$acc_bayes_mix[[ii]]/
                                               sample(mix.results.acc$acc_bayes_ca.tc[[ii]]))
    fit_quality$diff_acc_bayes_ia.tc[ii] <- list(mix.results.acc$acc_bayes_mix[[ii]]/
                                               sample(mix.results.acc$acc_bayes_ia.tc[[ii]]))
    fit_quality$diff_acc_bayes_single.tc[ii] <- list(mix.results.acc$acc_bayes_mix[[ii]]/
                                               sample(mix.results.acc$acc_bayes_single.tc[[ii]]))
  }
...
boot_ca_conserv <- calc_int_metric(results=fit_quality$diff_acc_boot_ca,conserv=TRUE)/sum(!is.na(fit_quality$diff_acc_boot_ca))
boot_ia_conserv <- calc_int_metric(results=fit_quality$diff_acc_boot_ia,conserv=TRUE)/sum(!is.na(fit_quality$diff_acc_boot_ia))
boot_single_conserv <- calc_int_metric(results=fit_quality$diff_acc_boot_single,conserv=TRUE)/sum(!is.na(fit_quality$diff_acc_boot_single))

boot_ca_accur <- calc_int_metric(results=fit_quality$diff_acc_boot_ca,conserv=F)/sum(!is.na(fit_quality$diff_acc_boot_ca))
boot_ia_accur <- calc_int_metric(results=fit_quality$diff_acc_boot_ia,conserv=F)/sum(!is.na(fit_quality$diff_acc_boot_ia))
boot_single_accur <- calc_int_metric(results=fit_quality$diff_acc_boot_single,conserv=F)/sum(!is.na(fit_quality$diff_acc_boot_single))

bayes_ca_conserv <- calc_int_metric(results=fit_quality$diff_acc_bayes_ca,conserv=TRUE)/sum(!is.na(fit_quality$diff_acc_bayes_ca))
bayes_ia_conserv <- calc_int_metric(results=fit_quality$diff_acc_bayes_ia,conserv=TRUE)/sum(!is.na(fit_quality$diff_acc_bayes_ia))
bayes_single_conserv <- calc_int_metric(results=fit_quality$diff_acc_bayes_single,conserv=TRUE)/sum(!is.na(fit_quality$diff_acc_bayes_single))

bayes_ca_accur <- calc_int_metric(results=fit_quality$diff_acc_bayes_ca,conserv=F)/sum(!is.na(fit_quality$diff_acc_bayes_ca))
bayes_ia_accur <- calc_int_metric(results=fit_quality$diff_acc_bayes_ia,conserv=F)/sum(!is.na(fit_quality$diff_acc_bayes_ia))
bayes_single_accur <- calc_int_metric(results=fit_quality$diff_acc_bayes_single,conserv=F)/sum(!is.na(fit_quality$diff_acc_bayes_single))

# repeat for TC library
boot_ca_conserv.tc <- calc_int_metric(results=fit_quality$diff_acc_boot_ca.tc,conserv=TRUE)/sum(!is.na(fit_quality$diff_acc_boot_ca.tc))
boot_ia_conserv.tc <- calc_int_metric(results=fit_quality$diff_acc_boot_ia.tc,conserv=TRUE)/sum(!is.na(fit_quality$diff_acc_boot_ia.tc))
boot_single_conserv.tc <- calc_int_metric(results=fit_quality$diff_acc_boot_single.tc,conserv=TRUE)/sum(!is.na(fit_quality$diff_acc_boot_single.tc))

boot_ca_accur.tc <- calc_int_metric(results=fit_quality$diff_acc_boot_ca.tc,conserv=F)/sum(!is.na(fit_quality$diff_acc_boot_ca.tc))
boot_ia_accur.tc <- calc_int_metric(results=fit_quality$diff_acc_boot_ia.tc,conserv=F)/sum(!is.na(fit_quality$diff_acc_boot_ia.tc))
boot_single_accur.tc <- calc_int_metric(results=fit_quality$diff_acc_boot_single.tc,conserv=F)/sum(!is.na(fit_quality$diff_acc_boot_single.tc))

bayes_ca_conserv.tc <- calc_int_metric(results=fit_quality$diff_acc_bayes_ca.tc,conserv=TRUE)/sum(!is.na(fit_quality$diff_acc_bayes_ca.tc))
bayes_ia_conserv.tc <- calc_int_metric(results=fit_quality$diff_acc_bayes_ia.tc,conserv=TRUE)/sum(!is.na(fit_quality$diff_acc_bayes_ia.tc))
bayes_single_conserv.tc <- calc_int_metric(results=fit_quality$diff_acc_bayes_single.tc,conserv=TRUE)/sum(!is.na(fit_quality$diff_acc_bayes_single.tc))

bayes_ca_accur.tc <- calc_int_metric(results=fit_quality$diff_acc_bayes_ca.tc,conserv=F)/sum(!is.na(fit_quality$diff_acc_bayes_ca.tc))
bayes_ia_accur.tc <- calc_int_metric(results=fit_quality$diff_acc_bayes_ia.tc,conserv=F)/sum(!is.na(fit_quality$diff_acc_bayes_ia.tc))
bayes_single_accur.tc <- calc_int_metric(results=fit_quality$diff_acc_bayes_single.tc,conserv=F)/sum(!is.na(fit_quality$diff_acc_bayes_single.tc))

# Bayesian prediction interval coverage of ACC

pred_ca_cover <- calc_predint_metric(results=mix.results.acc$acc_pred_ca)
pred_ca_cover.tc <- calc_predint_metric(results=mix.results.acc$acc_pred_ca.tc)

pred_ia_cover <- calc_predint_metric(results=mix.results.acc$acc_pred_ia)
pred_ia_cover.tc <- calc_predint_metric(results=mix.results.acc$acc_pred_ia.tc)

pred_single_cover <- calc_predint_metric(results=mix.results.acc$acc_pred_single)
pred_single_cover.tc <- calc_predint_metric(results=mix.results.acc$acc_pred_single.tc)

# compile metrics into tables
table1_acc_accurate <- data.frame(ca=c(100*point_fit_metric(fit_quality$diff_acc_ca,conserv=FALSE),
                                       100*boot_ca_accur,100*bayes_ca_accur,100*pred_ca_cover),
                                 ia=c(100*point_fit_metric(fit_quality$diff_acc_ia,conserv=FALSE),
                                      100*boot_ia_accur,100*bayes_ia_accur,100*pred_ia_cover),
                                 single=c(100*point_fit_metric(fit_quality$diff_acc_single,conserv=FALSE),
                                          100*boot_single_accur,100*bayes_single_accur,
                                          100*pred_single_cover),
                                 ca.tc=c(100*point_fit_metric(fit_quality$diff_acc_ca.tc,conserv=FALSE),
                                         100*boot_ca_accur.tc,100*bayes_ca_accur.tc,100*pred_ca_cover.tc),
                                 ia.tc=c(100*point_fit_metric(fit_quality$diff_acc_ia.tc,conserv=FALSE),
                                         100*boot_ia_accur.tc,100*bayes_ia_accur.tc,100*pred_ia_cover.tc),
                                 single.tc=c(100*point_fit_metric(fit_quality$diff_acc_single.tc,conserv=FALSE),
                                             100*boot_single_accur.tc,100*bayes_single_accur.tc,
                                             100*pred_single_cover.tc))

table2_acc_conserv <- data.frame(ca=c(100*point_fit_metric(fit_quality$diff_acc_ca),
                                      100*boot_ca_conserv,100*bayes_ca_conserv),
                                 ia=c(100*point_fit_metric(fit_quality$diff_acc_ia),
                                      100*boot_ia_conserv,100*bayes_ia_conserv),
                                 single=c(100*point_fit_metric(fit_quality$diff_acc_single),
                                          100*boot_single_conserv,100*bayes_single_conserv),
                                 ca.tc=c(100*point_fit_metric(fit_quality$diff_acc_ca.tc),
                                         100*boot_ca_conserv.tc,100*bayes_ca_conserv.tc),
                                 ia.tc=c(100*point_fit_metric(fit_quality$diff_acc_ia.tc),
                                         100*boot_ia_conserv.tc,100*bayes_ia_conserv.tc),
                                 single.tc=c(100*point_fit_metric(fit_quality$diff_acc_single.tc),
                                             100*boot_single_conserv.tc,100*bayes_single_conserv.tc))
```
...
for (jj in 1:nrow(mix.results.all)){
  ...
  if (mix.results.all$m4id_mix[jj] %in% mix.info$m4id_mix){
    ...
    sing_info <- mc5_attg_endpts[m4id==mix.results.all$potent_sing_id[jj],]
  ...
  if (mix.results.all$m4id_mix[jj] %in% mix.info.tc$m4id_mix){
    ...
    sing_info.tc <- mc5_attg_endpts[m4id==mix.results.all$potent_sing_id.tc[jj],]
